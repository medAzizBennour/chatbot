...
import logo from '../../img/linedata_logo.png';
...
const ChatBot = (): JSX.Element => {
  ...
  const startMicrophone = async () => {
    ...
    await audioContextRef.current.audioWorklet.addModule(DOWNSAMPLING_WORKER);
  ...
  const divs = useSelector(selectDivs);
  ...
  const commands = [
    ...
    {
      pattern: /(navigate|go) to (profile|analytics|dashboard)/i,
      command: (option: String, route: String) => {
        switch (route) {
          case 'profile':
            navigate('/profile');
            break;
  ...
  const executeCommand = (command: String) => {
    const matchingCommand = commands.find((cmd) => command.match(cmd.pattern));
    if (matchingCommand) {
    ...
    <div className='panel'>
      <div className='header'>
        ...
        <div className='header-text'>
          ...
          <p className='intro-header '>You're chatting with Linedata Bot</p>
        </div>
      </div>
        ...
        <div>
          {!recording ? (
            <IconButton
              ...
              children={<MicIcon sx={{ color: '#4b0082', fontSize: '30px' }} />}
            />
          ) : (
            <IconButton
